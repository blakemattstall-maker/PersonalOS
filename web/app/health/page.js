...
import { Page, PageHeader, Empty, Meta, SectionTitle } from "../ui.js";
    ...
    >
      {dates.map(date => {
        ...
        return (
          <Link
            key={date}
            href={date === today ? "/health" : `/health?date=${date}`}
            prefetch={false}
...
export default async function Health({ searchParams }) {
  ...
  const [data, log, body] = await Promise.all([
    ...
    safeGet("/api/health", { success: false, vitals: null })
  ]);
  ...
  return (
    <Page>
      ...
      <Reveal gap={70}>
        ...
        {/* Health is sectioned, not monolithic: Body first (it is the figure
            every other surface reasons from), Food below, and training slots
            in as a third section when there is training data to show. */}
        <div className="pos-reveal" data-reveal>
          <PageHeader title="Health">
            What your body is doing, and what you put in it.
          </PageHeader>
        </div>
