...
- **The overview names its islands.** Zoomed out, each connected component draws its most-connected member's name at its centroid ("Trifilm Exit Conversion", "Groceries"), fading out as node labels fade in. Components come from a union-find, titles from degree — no model, and no code change when islands merge.
