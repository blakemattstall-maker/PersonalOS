...
The whole graph, fullscreen, as a live force-directed canvas — the Obsidian-style view. This is a **decision of record that reversed an earlier one**: the first version was a deterministic radial diagram of one neighbourhood at a time, no layout library, and the user overruled it — the settle-into-clusters moment is the feature, for the demo and for daily use. The reversal became defensible exactly when the density work landed: at 37 edges a force layout is a dandelion; at ~300 with real hubs it is a net.

**The library is `force-graph` — the 2D one — and the choice is pinned by test.** Of the three candidates the user supplied (sigma.js, 3d-force-graph, cytoscape.js): Obsidian's own graph is 2D canvas, so the 2D sibling of 3d-force-graph gives the exact feel at ~80KB where three.js costs ~600KB and unreadable labels; sigma's WebGL earns its keep at tens of thousands of nodes and this graph is bounded at 5,000 edges by construction; cytoscape settles into analysis diagrams, not places. Same author and API as the 3D one if that is ever wanted.

How it holds the house rules:

- **Ember appears nowhere on the canvas** — a graph has nothing waiting on anybody. Fourteen types collapse into four families carried by the identity colours (people→iris, projects→tide, money→moss, notes→ink), which is exactly as many families as the palette has meanings.
- **Dot size is distinct-neighbour degree**, computed server-side in `fullGraph()` (`lib/links.js`) — the honest proxy for importance: derived from facts, no model, cannot flatter. Area grows with the square root, capped so a 43-neighbour hub is a landmark, not a moon.
- **The camera never trusts the physics, and the world is on a leash.** Nothing anchors the simulation to the viewport, and disconnected components repel each other for as long as it runs — the first builds settled half the net off a phone edge, then drifted apart for fifteen seconds. Three mechanisms fix it for good: weak `forceX/forceY` centring gathers islands into one compact world without crushing them (and needs no change when future data joins two islands); the whole graph is re-framed **every engine tick** until the user's first touch of the canvas, after which the camera never moves itself; and one final fit fires at engine stop, because `cooldownTime` is wall-clock and a backgrounded tab's frames pause while that clock runs. Deep links (`?type&id` from `ProjectCard`, `PersonCard`, `InsightCard`) still claim the camera and land centred and selected. Zoom is clamped both ways; a recentre button hands the camera back.
- **The overview names its islands.** Zoomed out, each connected component draws its most-connected member's name at its centroid ("Northline Print Exit Conversion", "Groceries"), fading out as node labels fade in. Components come from a union-find, titles from degree — no model, and no code change when islands merge.
- **Labels queue rather than smear.** A label appears when `zoom × √degree` crosses a threshold (hubs speak first), and each frame keeps a ledger of drawn label rectangles — a label that would land on one already drawn waits for a closer zoom. Priority is draw order, which is degree order. (`ctx.measureText` does the measuring; the pretext library the user pointed at turned out to be a text-layout utility, i.e. this exact job, already covered natively.)
- **Selection shows what a thing costs.** `moneyTotal()` in `phrasing.js` sums the absolute amounts of the selected node's direct transaction neighbours — a merchant's charges, a category's contents, a project's spend — in code, from figures the server attached. Absolute values, because the question is how much money moved.
- **One spread control instead of a bigger world.** `applySpread()` is the single place scale exists: repulsion, link length and collision padding move together off one slider, then the simulation re-settles. This is the growth architecture — as the graph gains nodes, the world spreads rather than the camera zooming ever further out.
- **The sphere exists — as an opt-in the flat view never pays for.** Declined once on cost grounds, then asked for twice, which is a decision. The 3D toggle dynamically imports `3d-force-graph` (three.js and all) on FIRST tap only; the everyday 2D page ships none of it, and a failed load falls back to flat at the cost of the tap. The layout is the same force simulation projected onto a sphere's surface every tick, it auto-rotates until first touch (orbit controls take over), the spread slider scales the sphere's radius, and the header caption names the hemisphere facing the camera. Selection, filters, the card and the caption run through the same refs in both views, so they cannot disagree about state.
- **Island titles live in the chrome, never on the canvas.** The first version drew them at component centroids and they clogged the overview — a title could be a whole intention sentence. Now a header caption (aria-live) names whichever island dominates the viewport (2D: most on-screen members; 3D: most camera-facing), computed both per-frame (throttled) and once at engine stop, because rendering pauses with the physics. Titles prefer a NAMEABLE member (project, person, merchant, category, place) over prose and are hard-truncated at 26 chars.
- **A charge's canvas label is its amount.** Its `label` column is its merchant's name, which the merchant node beside it already says — eight dots captioned "Fenwick Market" was noise wearing labels. Charges say "$62" and only at far closer zoom than named things.
- **Selection never yanks the camera.** The first version zoomed to 2.2× per tap; now zoom only nudges in (≤1.5×, never out), the node lands above the card's space, and the card is capped at a quarter of the screen.
- **Selection is the traversal surface.** Tapping a node highlights its neighbourhood and opens a card: type, degree, and every neighbour as a row phrased through `phrasing.js` — tapping a row walks the selection there. Family chips at the top are legend and filter in one; hiding a family only stops drawing it, never rebuilds the layout (a place must not rearrange because you squinted).
- **Reduced motion gets a settled graph**, not a slow-motion one: the simulation runs its ticks before first paint.
- **Disconnected islands are drawn as disconnected** — the grocery cluster genuinely does not touch the project cluster, and pretending otherwise would be an edge nobody stated.
- The engine arrives by dynamic import (it touches `window` at module scope); the data arrives as server-fetched props, one `fullGraph()` read serialised into the page.
