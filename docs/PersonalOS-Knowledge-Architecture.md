...
- **The 3D sphere was considered and declined, with the reversal path recorded.** The user proposed a spinnable sphere to solve getting lost in an expanseless plane; the bounded world, tick-framed camera, zoom clamps and spread control solve that in 2D without three.js's ~600KB, phone-battery WebGL, or angle-dependent label legibility. If the sphere is still wanted after living with this, `3d-force-graph` is the same author's API one import away.
