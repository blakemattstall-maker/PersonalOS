...
- **The camera never trusts the physics.** Nothing anchors the simulation to the viewport (the first render settled half the net off a phone's right edge), so the view fits-to-frame after warmup and again at engine stop — unless a `?type&id` deep link claimed the camera, in which case it centres and selects that node. The card links (`ProjectCard`, `PersonCard`, `InsightCard`) still arrive in context.
