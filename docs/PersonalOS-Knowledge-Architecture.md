...
**It is not a tab, but it has an unconditional door.** The bar holds six and stays that way; the card entries (`ProjectCard`, `PersonCard`, and `InsightCard` — which walks to the entity refs its detectors fired on) arrive pre-centred on the thing being looked at. But card entries alone shipped invisible: with no active project and no pending insight there was no way into the page at all. `GraphButton` in the root layout is the fix — a floating node-and-edges icon, top-right on every signed-in page, hidden on `/welcome` and `/login` by the same rule as the TabBar.
