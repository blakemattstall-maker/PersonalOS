...
- **Location-based automatic capture.** The location pipeline (`tools/location.js`) already labels places from visit patterns. It doesn't yet connect a visit to a place with any of the day's other activity — "you were at Kestrel Coworking for three hours and nothing got logged" is a nudge this system could plausibly make today with what it already tracks.
- **A real "what did I mean by that" resolver for ambiguous captures.** `tools/pending.js` handles the one-shot clarification case (router asks, next utterance answers). It doesn't handle a capture that references something from days ago ambiguously — "the thing with Priya" when there are three things with Priya on file. The entity graph (`lib/links.js`) already has everything needed to disambiguate by recency and connection strength; nothing currently calls it for this.
...
- **The islands graph as a real query surface**, not just a detector input. `lib/links.js` and `entity_links` already hold a genuine graph. Nothing today lets you ask "show me everything connected to this project" directly — every current consumer (the brief, the detectors) reads it implicitly. A `/graph` page or a `query_connections` tool would make the thing you can already see on the `/welcome` tour's demo scene into something real you can use.
