...
- ~~**Deepen the "Jarvis" personality further** … nothing yet notices that a `relationship_debt` finding and a `project_cost` finding are about the same project and worth saying together.~~ **Done Aug 9.** Findings carry typed entity refs and are grouped by shared entity (transitively) before phrasing, so one situation produces one sentence. The design question underneath it — *may an insight reference another insight's findings?* — was answered no, and the reasoning is in the Architecture doc §8 item 9: grouping is arithmetic, chaining is not.
...
- ~~**Should insights be allowed to reference each other's findings?**~~ **Decided Aug 9: no, with one distinction that resolves the tension.** Findings may be *grouped* by a shared entity before phrasing — that is arithmetic, the graph already knows whether two findings name the same project, and it is what turns two notifications into one sentence. A finding may not be *derived from* another finding's output, which is the thing that would let a chain of inferences sound more certain than any link in it. Group, then phrase once from raw facts. Enforced by a test that asserts no phrased body is ever a model input.
