...
### Aug 8, same day: that batch shipped two real bugs. Both are fixed; both are worth reading.

**Navigation and most buttons threw "This page couldn't load".** The boot splash script called `el.remove()` on `#pos-boot`, which React renders as part of the root layout and therefore owns. Removing it from outside React meant the next reconciliation of the layout hit a node that was gone and threw — and the layout reconciles on every `router.refresh()`, which is what nearly every server action here triggers. No `error.js` exists in this app, so it surfaced as Next's built-in error page, and a full reload always appeared to fix it because that rebuilt the node from HTML. It adds a class now and removes nothing. Since the element consequently stays in the tree for the life of the page, the hidden state also had to become genuinely inert (`pointer-events: none` plus a delayed `visibility: hidden`), and the script moved to `<head>`.

**`/practice` rendered a tall empty gap, and reveals generally felt late.** Both were the fractional `IntersectionObserver` threshold. `intersectionRatio` is measured against the element rather than the viewport, so an element taller than `viewport / threshold` can never reach the ratio however far it is scrolled — at 0.15 that was anything over ~4,400px, and `/practice` wrapped its entire topic list in one reveal target. It was not firing late; it was never going to fire. Everything now uses `threshold: 0` with a `rootMargin` inset, which is correct at any height, and `/practice` reveals per topic card like every other list.

**The lesson worth generalising from both:** each was invisible to the whole existing test suite, the build, and the linter, because each was a runtime interaction between correct-looking code and a browser API contract. Two tests now guard exactly these (`tests/welcome.test.js`), and both were confirmed to fail against the old code before being kept.

Two smaller things fixed in the same pass: a hydration mismatch on every donut arc (`Math.cos`/`Math.sin` are not required to be correctly rounded, so Node and the browser disagreed in the last bits once `MoneyCharts` became a client component — coordinates are rounded to 3dp now), and an 8s failsafe on the welcome intro's handover, because the hero is CSS-hidden until the intro finishes and anime.js stops advancing while the document is hidden, so backgrounding the tab mid-intro previously stranded the whole page invisible.
