...
**Known live issue as of Aug 6, still open:** `finance_cache` writes (INSERT/UPDATE/DELETE) are failing with `PGRST205` ("table not found in schema cache") while reads against the same table succeed — a Supabase/PostgREST infrastructure asymmetry that has persisted well past normal schema-cache propagation time (confirmed via raw REST calls, not just supabase-js, and confirmed still failing after real time had passed, not just a few seconds). `getFinancialData()` degrades safely when this happens — falls back to always-live fetch, exactly the pre-caching behaviour, confirmed nothing breaks user-facing.

**The actual fix, not a UI hunt:** Blake couldn't locate a "Reload schema cache" button in the Supabase dashboard — the reliable fix doesn't need one. Run this in the **same SQL editor already used for every migration** (Supabase Dashboard → SQL Editor → New query):
```sql
NOTIFY pgrst, 'reload schema';
```
This is PostgREST's own documented mechanism for forcing a schema reload and works regardless of which dashboard version/layout is current — don't go hunting through Settings pages for a button that may have moved or been renamed. If it's still broken after that, the next things to check are RLS policies on `finance_cache` specifically (service role should bypass RLS by default, but confirm no policy was accidentally added) and whether the table genuinely has `INSERT`/`UPDATE`/`DELETE` grants for the role the service key uses. Re-verify the write path directly before assuming it's fixed:
```bash
...
- Resolve the `finance_cache` write-path issue — the fix is known (`NOTIFY pgrst, 'reload schema';` in the SQL editor), just needs to actually be run and re-verified (see Money, above).
